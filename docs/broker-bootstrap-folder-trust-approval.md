...
- **runtime 層の配線**: dispatcher の action plan を生成する runtime helper
  ...
  **`send_keys(target=worker, enter=True)` を 1 回**入れる
  ...
  - **既知ドリフト (本タスクで src 非変更)**: この after_spawn の `reason` は
    `"approve 'Load development channel?' Y/n prompt"` と **renga 旧文言のまま**で、
    broker では実際には folder-trust プロンプトを承認する。挙動 (Enter 送出) は
    正しく、文言のみのドリフト。option A (src 挙動非変更) のため本タスクでは
    修正せず申し送りとする (§5)。
...
- **`dispatcher.runner` after_spawn の reason 文言ドリフト (申し送り)**:
  `src/claude_org_runtime/dispatcher/runner.py:689` の `send_keys` step は
  reason が `"approve 'Load development channel?' Y/n prompt"` と renga 旧文言の
  ままで、broker では実際 folder-trust を承認する。挙動 (Enter 送出) は正しく
  文言のみのドリフト。本タスクは option A (src 挙動非変更) のため未修正。broker
  既定化が進んだ段で reason を folder-trust 向けに更新する follow-up を推奨
  (cosmetic、別 commit / 別タスク)。
